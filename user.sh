...
unzip -o /tmp/user.zip &>> $LOGS_FILE    # o - means overwriting if user is already unzipped it will overwrite those files or you can remove entire app rm -rf /app/*
...
VALIDATE $? "copying the user service and updated mongodb DNS record"
...
VALIDATE $? "Enable and start the user service"
